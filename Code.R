# R Script Graf_et_al_2019_R: R-Code
# Nadin Graf, Karina P. Battes, Mirela Cimpean, Martin H. Entling, Katharina Frisch,
# Moritz Link, Andreas Scharmueller, Verena C. Schreiner,
# Eduard Szoecs, Jochen P. Zubrod, Ralf B. Schaefer
#----------------------------------------------------------------------------------------
# Relationship between agricultural pesticides and
# the diet of riparian spiders
#----------------------------------------------------------------------------------------
...
# 76829 Landau
# GERMANY
# graf-nadin@uni-landau.de, schaefer-ralf@uni-landau.de
...
jags.1 <- run_model(
  run = "extreme", mix, source, discr, model_filename,
...
trap_days <- read.table("emergence_trap_days.csv", header = TRUE, sep = ",", na.strings = c("NR", "NA"))
aquatic_abundance <- read.table("aquatic_abundance.csv", header = TRUE, sep = ",", na.strings = c("NR", "NA"))
aquatic_abundance$number <- as.numeric(aquatic_abundance$number)
biomass_estimation <- read.table("biomass_estimation.csv", header = TRUE, sep = ",", na.strings = c("NR", "NA"))
...
biomass_estimation <- unite(biomass_estimation, "combi", "order", "suborder", "family", "genus", "species", remove = FALSE)
...
# and relevant for spiders and their aquatic prey

proxy_vars <- c(
  "cl", "ec_ms", "ph", "no3", "po4", "so4", "o2", "shore_cover_shrubs",
  "shore_cover_meadow", "fid_factor", "land_use_meadows",
  "land_use_agri", "totalarea", "ext_agri", "int_agri", "max_width", 
...
mas1 <- glm(biomass ~ max_sumTU_ms + Comp.1 + Comp.2, mas, family = "gaussian")
summary(mas1) #-> exclude comp2
drop1(mas1)

mas2 <- glm(biomass ~ max_sumTU_ms + Comp.1, mas, family = "gaussian")
summary(mas2) #-> exclude tox
drop1(mas2)

mas3 <- glm(biomass ~ Comp.1, mas, family = "gaussian")
summary(mas3)
plot(mas$biomass ~ mas$max_sumTU_ms)
1 - (285141 / 415687) # D = 0.314
AIC(mas3)

masa <- glm(biomass ~ no2, mas, family = 'gaussian')
summary(masa)
masb <- glm(biomass ~ nh4, mas, family = 'gaussian')
summary(masb)  
masc <- glm(biomass ~ no3, mas, family = 'gaussian')
summary(masc) 
...
cor.test(tet$Comp.1, tet$max_sumTU_ms)
cor.test(tet$biomass, tet$max_sumTU_ms)
cor.test(tet$Comp.2, tet$max_sumTU_ms)
cor.test(tet$Comp.2, tet$biomass)
cor.test(tet$Comp.2, tet$Comp.1)

# stepwise selection
tet1 <- lm(tet_aqu_diet ~ max_sumTU_ms + Comp.1 + Comp.2 + biomass, tet)
summary(tet1)
drop1(tet1) #-> exclude Comp.2

tet2 <- lm(tet_aqu_diet ~ max_sumTU_ms + Comp.1 + biomass, tet)
summary(tet2)
library(car)
Anova(tet2, type = 2)
AIC(tet2)
...
# model for abundance of Tetragntha sp.
numtet <- glm(num_tet ~ max_sumTU_ms, family = "quasipoisson", dat)
summary(numtet)
1 - (40.736 / 46.929) #-> D = 0.132
...
svg("mass.svg", width = 5.7, height = 5.4)
plot(predictorEffects(tet2, "biomass"),
  lines = list(col = {
    "black"
  }),
  main = paste(""),
  ...
  axes = list(
    y = list(
      lim = (c(0.38, 0.62)),
      ticks = list(at = c(0.4, 0.45, 0.50, 0.55, 0.6)),
      lab = {
    ...
    x = list(
      biomass = list(
        lim = (c(40, 610)),
        ticks = list(at = c(0, 100, 200, 300, 400, 500, 600)),
        lab = "Aquatic prey dry biomass [mg]"
      ),
      rug = TRUE,
      cex = 1.2
    )
...
svg("tox.svg", width = 5.7, height = 5.4)
plot(predictorEffects(tet2, "max_sumTU_ms"),
  lines = list(col = {
    "black"
  }),
  main = paste(""),
  ...
  axes = list(
    ...
    x = list(
      max_sumTU_ms = list(
        lim = (c(-1.45, 0.1)),
        ticks = list(at = c(-1.4, -1.2, -1, -0.8, -0.6, -0.4, -0.2, 0)),
        lab = "Toxicity [max sumTU]"
      ),
      rug = TRUE,
      cex = 1.2
    )
...
svg("spca.svg", width = 5.7, height = 5.4)
plot(predictorEffects(tet2, "Comp.1"),
  lines = list(col = {
    "black"
  }),
  main = paste(""),
  ...
  axes = list(
    ...
    x = list(
      Comp.1 = list(
        lim = (c(-3.5, 5.5)),
        ticks = list(at = c(-3, -2, -1, 0, 1, 2, 3, 4, 5)),
        lab = "First SPCA axis"
      ),
      rug = TRUE,
      cex = 1.2
...
cor.test(par$max_sumTU_ms, par$Comp.2) 
cor.test(par$max_sumTU_ms, par$Comp.1) 
cor.test(par$biomass, par$max_sumTU_ms) 
cor.test(par$biomass, par$Comp.2) 
cor.test(par$biomass, par$Comp.1)

par1 <- glm(par_aqu_diet ~ max_sumTU_ms + Comp.1 + Comp.2 + biomass, par, family = "gaussian")
summary(par1)
drop1(par1) #-> exclude tox

par2 <- glm(par_aqu_diet ~ biomass + Comp.1 + Comp.2, par, family = "gaussian")
summary(par2)
drop1(par2) #-> exclude biomass

par3 <- glm(par_aqu_diet ~ Comp.1 + Comp.2, par, family = "gaussian")
summary(par3)
drop1(par3) #-> exclude comp.2

par4 <- glm(par_aqu_diet ~ Comp.1, par, family = "gaussian")
summary(par4)

# model for abundance of Pardosa sp.
numpar <- glm(num_par ~ max_sumTU_ms, family = "quasipoisson", dat)
summary(numpar)
plot(numpar)
1 - (142.65 / 273.21) #-> D = 0.478
142.65 / 15
